...
- `SYSTEM_PROMPT` inneholder eksplisitte språkkrav: korrekt bokmål, unngå anglisismer («tangentielt» → «perifert berørt», «hovemotiv» → «hovedmotiv»); to kategorietagger: `teamledelse`, `personalledelse`; seks tematagger: `feedback`, `kultur`, `rekruttering`, `motivasjon`, `coaching`, `endringsledelse`
