...
- `update_podcasts.py` — RSS fetcher; adds new episodes (Rating=0) since last known date per podcast. Dedup bruker fire nøkler i lag: `(podcast, tittel)` → eksakt lenke (kun unike lenker) → `(podcast, plattform-ID)` via `extract_episode_id()` (UUID eller numerisk ID ≥6 siffer foran slug; scopet per podcast for å unngå falske positive på tvers av shows) → `(podcast, dato)` som siste sikkerhetsnett, kun for plattformer uten stabil ID. Lenke hentes i prioritert rekkefølge: `<link>` → `<enclosure url>` → `<guid>`. CSV skrives dato-sortert (nyest først).
...
- `embed_csv.py` — skriver CSV-innholdet inn i HTML-filens `data`- og `archiveData`-array; kjøres etter hver raterunde. Filtrerer ut Rating=0-episoder og sorterer dato descending (nyest først) før embedding
