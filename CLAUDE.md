...
- **Mobil sveip-til-favoritt**: sveip høyre (>60px) på et episodekort toggler favoritt-stjernen — event delegation via `touchstart`/`touchend` på `#tableBody` (passive listeners); `tr.dataset.fav` settes i `renderTable()` for identifikasjon; `.fav-flashed`-klasse trigger gul glimt-animasjon (`@keyframes fav-flash`) som visuell bekreftelse
